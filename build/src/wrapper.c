...
// Length of nul-terminated string in bytes.
...
// Start and length of buffer to read from it.
char* KEEPALIVE buffer_bytes(buffer* buf) {
  return buf->bytes;
}
int KEEPALIVE buffer_len(buffer* buf) {
  return buf->size;
}

#ifdef DEBUG_BUILD
extern buffer* debug_log;
int KEEPALIVE test_debug() {
  if (!debug_log)
    debug_log = new_buffer();
  return write_buffer(debug_log, "Test!", 0, 5);
}
#endif
...
int KEEPALIVE init() {
  ...
  // Initialize SQLite
  last_status = sqlite3_open(db_path, &db);
